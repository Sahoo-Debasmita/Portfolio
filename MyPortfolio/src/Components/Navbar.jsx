...
const Link=[
  ...
  return (
    ...
    <ul className='hidden md:flex'>
      {Link.map(({id,link})=>(
 <li key={id} className='px-4 text-lg cursor-pointer font-medium text-gray-500 capitalize hover:scale-105 duration-200'>{link}</li>
      ))}
    </ul>
    ...
    {hamburger  && ( 
       ...
       {Link.map(({id,link})=>(
    <li key={id} className='capitalize py-4 text-4xl cursor-pointer px-4 hover:scale-105 duration-200'>{link}</li>
         ))}
       </ul>
    ) }
